...
class bSkinSaverUI(QtWidgets.QDialog):
    def __init__(self, parent=getMayaWindow()):
        super(bSkinSaverUI, self).__init__(
        ...
        self.selectVerticesFileButton = QtWidgets.QPushButton(
            "Set File", parent=self)
        ...
        # select files
        self.connect(self.selectObjectsFileButton, QtCore.SIGNAL(
            "clicked()"), self.selectObjectsFile)
        self.connect(self.selectVerticesFileButton, QtCore.SIGNAL(
            "clicked()"), self.selectVerticesFile)

        self.connect(self.saveObjectsButton, QtCore.SIGNAL(
            "clicked()"), self.saveObjects)
        self.connect(self.loadObjectsButton, QtCore.SIGNAL(
            "clicked()"), self.loadObjects)
        self.connect(self.loadObjectsSelectionButton, QtCore.SIGNAL(
            "clicked()"), self.loadObjectsSelection)

        self.connect(self.saveVerticesButton, QtCore.SIGNAL(
            "clicked()"), self.saveVertices)
        self.connect(self.loadVerticesButton, QtCore.SIGNAL(
            "clicked()"), self.loadVertices)
...
def bLoadVertexSkinValues(inputFile, ignoreJointLocks):
    ...
    for i in range(infCount):
        mayaJoints.append(OpenMaya.MFnDagNode(
            influenceArray[i]).name().split('|')[-1])
        ...
        for k in range(len(allJoints)):
            ...
            if k < len(fileJoints):
                if not allLocks[k]:
                    ...
                    weightDoubles.append(fileWeightFloats[i][k])
                else:
                    if allExistInMaya[k]:
                        weightDoubles.append(
                            oldWeightDoubles[allExistInMaya[k] + len(mayaJoints) * i])
                    else:
    ...
    for i in range(bindVertCount):
        ...
        if sumA > 0.0001:
            ...
            for inf in range(len(allJoints)):
                if not allLocks[inf]:
                    weightDoubles[inf + i *
                                  len(allJoints)] *= sumADenom * (1-sumB)
    ...
    if doSoftSelection:
        for i in range(bindVertCount):
            for inf in range(len(allExistInMaya)):
                ...
                weightDoubles[index] = weightDoubles[index] * \
                    softWeights[i] + oldWeights * (1.0-softWeights[i])
    ...
    fnSkinCluster.setWeights(bSkinPath, vtxComponents,
                             allJointsIndices, weightDoubles, 0)
...
def bSaveVertexSkinValues(inputFile, ignoreSoftSelection):
    ...
    for i in range(len(vertIds)):
        ...
        """
        weightsString = ' '.join(['0' if x == 0 else str(x) for n, x in enumerate(
            WeightArray[i*infCount: (i+1)*infCount]) if weightCheckArray[n]])
...
def bSaveSkinValues(inputFile):
    ...
    while not iterate.isDone():
        ...
        if not node.hasFn(OpenMaya.MFn.kTransform):
            print('{} is not a Transform node (need to select transform node of polyMesh)'.format(
                ...
                if childObject.hasFn(OpenMaya.MFn.kMesh) or childObject.hasFn(OpenMaya.MFn.kNurbsSurface) or childObject.hasFn(OpenMaya.MFn.kCurve):
                    ...
                    if skinCluster is not False:
                        ...
                        for k in range(influentsCount):
                            ...
                            jointTokens = jointTokens[len(
                                jointTokens)-1].split(':')
                            output.write(
                                jointTokens[len(jointTokens)-1] + '\n')
                        ...
                        for i in range(vertexCount):
                            ...
                            saveString = ' '.join(['0' if x == 0 else str(
                                x) for n, x in enumerate(WeightArray[i*infCount: (i+1)*infCount])])
    ...
    output.close()
    print('done saving weights, it took ',
          (time.time()-timeBefore), ' seconds.')
...
def bSkinObject(objectName, fileJoints, weights):
    ...
    for jointIndex in range(len(fileJoints)):
        ...
        it = OpenMaya.MItDependencyNodes(OpenMaya.MFn.kJoint)
        while not it.isDone():
            sceneJointTokens = str(OpenMaya.MFnDagNode(
                it.item()).fullPathName()).split('|')
    ...
    bSkinPath = OpenMaya.MDagPath()
    fnSkinCluster.getPathAtIndex(
        fnSkinCluster.indexForOutputConnection(0), bSkinPath)
    ...
    #
    fnSkinCluster.setWeights(bSkinPath, vtxComponents,
                             mayafileJointsMapArray, weightDoubles, 0)
...
def bLoadSkinValues(loadOnSelection, inputFile):
            ...
            if node.hasFn(OpenMaya.MFn.kTransform):
                ...
                if NewTransform.childCount():
                    if NewTransform.child(0).hasFn(OpenMaya.MFn.kMesh):
                        PolygonObject = str(OpenMaya.MFnDagNode(
    ...
    print('done loading weights, it took ',
          (time.time()-timeBefore), ' seconds.')
...
def getSoftSelection():
    ...
    iter = OpenMaya.MItSelectionList(
        selection, OpenMaya.MFn.kMeshVertComponent)
    elements, weights = [], []
    while not iter.isDone():
        ...
        def getWeight(i): return fnComp.weight(
